...
import styles from "./FormEditProducto.module.css";
...
interface FormEditProductoProps {
  onClose: () => void; // Prop para cerrar el formulario
  categorias: ICategorias[];
  producto: IProductos;
}
export const FormEditProducto: FC<FormEditProductoProps> = ({
  onClose,
  categorias,
  producto,
}) => {
  const [failTry, setFailTry] = useState<boolean>(false);
  const [urlResponce, setUrlResponce] = useState<string | null>(
    producto.imagenes.length > 0 ? producto.imagenes[0].url : null
  );

  const [isHabilitado, setIsHabilitado] = useState<boolean>(
    producto.habilitado
  );
  const [categoria, setCategoria] = useState<ICategorias | null>(
    producto.categoria
  );
  const [alergenosSelected, setAlergenosSelected] = useState<IAlergenos[]>(
    producto.alergenos
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newProducto: IUpdateProducto = {
      ...
      idAlergenos:
        alergenos.length > 0 ? alergenosSelected.map((ale) => ale.id) : [],
    ...
    try {
      await productoService.updateProducto(producto.id, newProducto);
      onClose();
    } catch (error) {
  ...
  const handleCheckboxChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setAlergenosSelected(
      (prevSelected) =>
        e.target.checked
          ? [...prevSelected, alergenos.find((ale) => ale.id === value)!] // Agrega el alérgeno seleccionado
          : prevSelected.filter((ale) => ale.id !== value) // Elimina el alérgeno desmarcado
    );
  };
                  ...
                  {categorias.map((cat) => {
                    return (
                      <option
                        ...
                        selected={categoria !== null && cat.id === categoria.id ? true : false}
                      >
